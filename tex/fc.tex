...
%		One approach is to again parallelize the $\KwLet{}.\KwLet{}$ reduction and
%		refine the order on $\KwLet{}.\KwLet{}$ by self-labeling
%		ordered by $\KwLet{}.\S$
%		($\KwLet{}.\KwLet{}^e > \KwLet{}.\KwLet{}^{e'}$ iff $e \xrightarrow{\KwLet{}.\S}^+ e'$),
\end{enumerate}
% par(λ.v, let.v, d.v, d.S, k.S, let.let, A.d, A.v) > selford(let.S, dl.S)
% seems to work
% An idea to try is ordering by the depth at which reduction occurs.
% wrong: not that simple because of the let.S-let.S case, need to refine
% depth relative to some delimiter also not right because of k.S-let.S case
...
\begin{align*}
	...
	&((λx.\,e)\:v)^● = e^●\subst{x}{v^●} \\
	&(u\:v)^● = u^●\:v^●\\
	&(\Let{x}{e}{t})^● = \mklet{e^●}{(x.\,t^●)} \\
	&(\S κ.\,e)^● = \S κ.\,e^● \\
	&⟨u/v⟩^● = u^● \\
	&⟨\S κ.\,e/v⟩^● = e^●\subst{κ}{⟨□/v^●⟩} \\
	&⟨e/v⟩^● = ⟨e^{●/v}/v^●⟩ \\
	&κ⟦\S κ'.\,e⟧^● = e^●\subst{κ'}{κ} \\
	% WRONG: e●{κ:=κ[let x = □ in t●]} -/→* (e{κ:=κ[let x = □ in t]})●
	&κ⟦e⟧^● = κ⟦e^●⟧ \\
	&\Ask(κ)^● = \Ask(κ) \\
\\
	&(\Let{x}{e}{t})^{●/v} = \mklet{e^●}{(x.\,t^{●/v})} \\
	&(\S κ.\,e)^{●/v} = \A\,e^●\subst{κ}{⟨□/v^●⟩} \\
	&(\A⟨e/v⟩)^{●/v} = e^{●/v} \\
	&(\A\,u)^{●/v} = u^● \\
	&e^{●/v} = e^● \\
\\
	&\mklet{v}{x.\,t} = t\subst{x}{v} \\
	&\mklet{(\S κ.\,e)}{x.\,t} = \S κ.\,e\subst{κ}{κ⟦\Let{x}{□}{t}⟧} \\
	&\mklet{(\Let{y}{e}{t'})}{x.\,t} = \Let{y}{e}{\mklet{t'}{x.\,t}} \\
	&\mklet{e}{x.\,t} = \Let{x}{e}{t} \\
\end{align*}

\caption{
	Complete development $·^●$ and its tail-of-delimiter variant $·^{●/v}$.
	It's a \textit{super}development for reductions with $\KwLet{}$ at the root,
	meaning we develop subterms first and then the helper $\textsf{mklet}$ performs reductions.
	The value of a function is determined by the first pattern that matches, as in Haskell.
}

\end{figure}

\begin{lemma}[Extensivity]
	$a →^* a^●$
\end{lemma}

\begin{theorem}[Z property]
	If $a → b$, then $b →^* a^● →^* b^●$.
	%and $b →^* a^{●/v} →^* b^{●/v}$.
	% FALSE: would need to have defined tail reductions →/v
\end{theorem}
\begin{proof}
	We consider reduction at the root.
	\begin{enumerate}
		\item $(λx.\,e)\,v → e\subst{x}{v} →^* ((λx.\,e)\,v)^● = e^●\subst{x}{v^●} →^* (e\subst{x}{v})^●$
		\item $\Let{x}{v}{t} → t\subst{x}{v} →^* (\Let{x}{v}{t})^● = t^●\subst{x}{v^●} →^* (t\subst{x}{v})^●$
		\item $⟨v╱u⟩ → v →^* ⟨v/u⟩^● = v^●$
		\item $\Let{x}{\S κ.\,e}{t} → \S κ.\,e\subst{κ}{κ⟦\Let{x}{□}{t}⟧} →^* (\Let{x}{\S κ.\,e}{t})^● = \S κ.\,e^●\subst{κ}{κ⟦\Let{x}{□}{t^●}⟧} →^* (\S κ.\,e\subst{κ}{κ⟦\Let{x}{□}{t}⟧})^●$
		\item $⟨\S κ.\,e ╱ v⟩ → e\subst{κ}{⟨□╱v⟩} →^* (⟨\S κ.\,e ╱ v⟩)^● = e^●\subst{κ}{⟨□/v^●⟩} →^* (e\subst{κ}{⟨□/v⟩})^●$
		\item $κ⟦\S κ'.\,e⟧ → e\subst{κ'}{κ} →^* e^●\subst{κ'}{κ} = (e\subst{κ'}{κ})^●$
		\item $⟨L[\S κ.\,e]╱v⟩ → ⟨L[\A\,e\subst{κ}{⟨□╱v⟩}]╱v⟩$ \\
			If $L=□$, we have $⟨\S κ.\,e/v⟩ → ⟨\A\,e\subst{κ}{⟨□/v⟩}/v⟩ →^* e^●\subst{κ}{⟨□/v^●⟩} →^* (e\subst{κ}{⟨□/v⟩})^●$. \\
			Otherwise, we have $⟨L[\S κ.\,e]╱v⟩^● = ⟨L^●[\A\,e^●\subst{κ}{⟨□/v^●⟩}]/v^●⟩ →^* ⟨L[\A\,e\subst{κ}{⟨□/v⟩}])/v⟩^●$.
		\item $⟨L[\A⟨e╱v⟩]╱v⟩ → ⟨L[e]╱v⟩$ \\
			If $L=□$, we have $⟨\A⟨e/v⟩/v⟩ → ⟨e/v⟩ →^* ⟨\A⟨e/v⟩/v⟩^● = ⟨e/v⟩^●$. % a →* a●
		\item $⟨L[\A\,u]╱v⟩ → ⟨L[u]╱v⟩$ \\
			If $L=□$, we have $⟨\A\,u/v⟩ → ⟨u/v⟩ →^* ⟨\A\,u/v⟩^● = u^● = ⟨u/v⟩^●$. % a →* a●
		\item $\Let{x}{\Let{y}{e}{t_1}}{t_2} → \Let{y}{e}{\Let{x}{t_1}{t_2}}$
	\end{enumerate}
	\begin{enumerate}
		\item $\Let{x}{v}{t} → t\subst{x}{v} →^* (\Let{x}{v}{t})^{●/u} = t^{●/u}\subst{x}{v^●} →^* (t\subst{x}{v})^{●/u}$
		\item $\Let{x}{\S κ.\,e}{t} → \S κ.\,e\subst{κ}{κ⟦\Let{x}{□}{t}⟧} →^* (\Let{x}{\S κ.\,e}{t})^{●/v} = \S κ.\,e^●\subst{κ}{κ⟦\Let{x}{□}{t^{●/v}}⟧} \not→^* (\S κ.\,e\subst{κ}{κ⟦\Let{x}{□}{t}⟧})^{●/v}$
			% FALSE
	\end{enumerate}

	Now, the inductive cases.
	\begin{enumerate}
		\item $λx.\,a → λx.\,b →^* λx.\,a^● →^* λx.\,b^●$
		\item $a\:v → b\:v →^* a^●\:v^● →^* b^●\:v^●$
		\item $x\:a → x\:b →^* x\:a^● →^* x\:b^●$
		\item $(λx.\,e)\:a → (λx.\,e)\:b →^* e\subst{x}{a^●} →^* e\subst{x}{b^●}$
		\item $⟨a/v⟩ → ⟨b/v⟩$.
			By IH, $a → b →^* a^● →^* b^●$.
	\end{enumerate}
\end{proof}
