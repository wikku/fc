...
%The result of the substituE and the following simplification are:
...
%		e\;v &≡ \Let{x}{e}{x\;v} \tag{application to expr}\\
...
The rule $\KwLet{}.\S$ has utility beyond simulating the coarse-grained evaluation steps.
...
%By the $\KwLet{}.\KwLet{}$ and $\KwLet{}.\S$ rules, every $\S$
...
consider the following snippet which checks satisfiability of a Boolean function $f$.
When the operation asks for a boolean,
the handler
$h ≡ λx\,k.\,k\,\true ││ k\,\false$
tries both options (simulating angelic nondeterminism).

\begin{align*}
  &  ⟨\Let{\_}{\Log\,"working"}\Let{x}{\Op()}{\Let{y}{\Op()}{f(x,y)}} ╱ h ⟩
→\quad &
  ⟨\Log\,"working";\S κ.\,\Ask(κ)\,()\,(λz.\,κ[\Let{x}{z}{\Let{y}{\Op()}{f(x,y)}}]) ╱ h ⟩ \\
→^*\quad
  &  ⟨\Log\,"working";\S κ.\,\Ask(κ)\,()\,(λz.\,κ[\Let{y}{\Op()}{f(z,y)}]) ╱ h ⟩ \\
→^*\quad
  &  ⟨\Log\,"working";\A\,(
        ⟨\Let{y}{\Op()}{f(\true,y)} ╱ h⟩
     ││ ⟨\Let{y}{\Op()}{f(\true,y)} ╱ h⟩) ╱ h⟩ \\
→^*\quad
  &  ⟨\Log\,"working";\A\,(
...
%With $\KwLet{}.\S$ and $dL.\S$ we can derive the coarse-grained rewrites that started it all:
...
\begin{mathpar}
	...
	\inferrule{L \Int L' \\ e \Int e' \\ v \Int v' }
	{⟨L[\S κ.\,e]╱v⟩ \Int ⟨L'[\A\,e'\subst{κ}{⟨□╱v'⟩}]╱v'⟩}

	\inferrule{L \Int L' \\ e \Int e' \\ v \Int v' }
	{⟨L[\A\,⟨e╱v⟩]╱v⟩ \Int ⟨L'[e']╱v'⟩}

	\inferrule{L \Int L' \\ e \Int e' \\ v \Int v' \\ u \Int u' }
	{⟨L[\A\,u]╱v⟩ \Int ⟨L'[u']╱v'⟩}
\end{mathpar}
...
The parallel reduction has two modes (ranged over by $M$): \textit{g}eneral and tail of \textit{d}elimiter.
...
Formally, the $\KwLet{}.\S$ reduction is allowed only for impure lets (without the assertion)
