...
\\ \\
%\begin{align*}
	&\mkapp{(λx.\,e)}{v} = e\subst{x}{v} \\
	&\mkapp{u}{v} = u\:v \\
	&\\
	&\mklet{v}{x.\,t} = t\subst{x}{v} \\
	&\mklet{(\S κ.\,e)}{x.\,t} = \S κ.\,e\subst{κ}{κ⟦\mklet{□}{x.\,t}⟧} \\
	&\mklet{(\Let{y}{e}{t'})}{x.\,t} = \Let{y}{e}{\mklet{t'}{x.\,t}} \\
	&\mklet{e}{x.\,t} = \Let{x}{e}{t} \\
	&\\
	&\mkdelim{v}{u} = v \\
	&\mkdelim{(\S κ.\,e)}{u} = e\subst{κ}{\mkdelim{□}{u}} \\
	&\mkdelim{L[\S κ.\,e]}{u} = ⟨L[\A\,e\subst{κ}{\mkdelim{□}{u}}]/u⟩ \\
	&\mkdelim{L[\A⟨e/u⟩]}{u} = ⟨L[e]/u⟩ \\
	&\mkdelim{L[\A\,v]}{u} = ⟨L[v]/u⟩ \\
	&\mkdelim{e}{u} = ⟨e/u⟩ \\
	&\\
	&\mkplug{κ}{\S κ'.\,e} = e\subst{κ'}{κ} \\
	&\mkplug{κ}{e} = κ⟦e⟧
\end{align*}

\caption{
	Complete superdevelopments.
	The functions are defined by the first pattern that matches.
}
...
\begin{proof}
	Induction on $a → b$.
	\begin{enumerate}
		\item $(λx.\,e)\,v → e\subst{x}{v} \stackrel{IH}{→^*} ((λx.\,e)\,v)^● = e^●\subst{x}{v^●} →^* (e\subst{x}{v})^●$
		\item $\Let{x}{v}{t} → t\subst{x}{v} \stackrel{IH}{→^*} (\Let{x}{v}{t})^● = t^●\subst{x}{v^●} →^* (t\subst{x}{v})^●$
		\item $⟨v╱u⟩ → v →^* ⟨v/u⟩^● = v^●$
		\item $\Let{x}{\S κ.\,e}{t} → \S κ.\,e\subst{κ}{κ⟦\Let{x}{□}{t}⟧}$
		\item $⟨\S κ.\,e ╱ v⟩ → e\subst{κ}{⟨□╱v⟩}$
