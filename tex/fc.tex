...
\author{Wiktor Kuchta}{affiliation}{wk@email.com}
\title{A Fine Calculus for Static Delimited Control}
...
\subsection{Structural substitution}
...
\subsubsection{Capturing the static delimiter out of order}
...
\subsubsection{Eta-like reductions}
...
\subsubsection{Let reassociation}
...
The previous section has demonstrated how reduction can be used for optimization.
...
\subsection{Internal parallel reduction}
...
\section{Abella mechanization}

We have formalized adequacy in the Abella proof assistant.
We build on abel's third order representation of the $λμ$-calculus.
Abel noted issues with defining parallel reductions in Twelf,
but they are overcome by Abella's nabla-quantification,
an approach similar to Accattoli.
There is a notable, yet benign difference in the formalization:
We have an expression $\Ask(D)$ instead of a value $\Ask(κ)$,
and instead of extending structural substitution we have the following evaluation rule:
$$\Ask(⟨E╱v⟩) ↦ v$$

\section{Related work}

\subsection{Distributing delimiters}
...
is no delimiter, but we can (subsubsection \ref{strred}).

\subsection{shift0 axiomatization}
Here we compare some of the equational axioms of $\shiftz$ by Materzok \cite{materzok}.

\subsubsection*{shift0/reset ($λ_{\S_0}$)}
...
\subsubsection*{shift0/dollar ($λ_\$$)}
...
\subsection{Delimited \texorpdfstring{$λμ$}{lambda-mu}}
...
\subsection{Fine-grained syntax for algebraic operations}
...
\subsection{Expressing handlers using shift0}
...
\subsection{Expressing return clauses}
...
\subsection{Tail reductions and letcc}
...
\subsection{Purity-aware reductions}
...
\subsection{Multi-prompt (labeled) control}
