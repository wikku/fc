...
\newcommand{\foreign}[1]{\textcolor{blue}{#1}}
...
\begin{enumerate}
	\item
		...
		denote plugging the expression $e$ into context $D$ on the meta level.
...
consider the following snippet that checks satisfiability of a Boolean function $f$.
When the operation asks for a boolean, the handler tries both options (simulating angelic nondeterminism).

\begin{align*}
  &  \Handle\\
  &  \quad\Log\,"working";\Let{x}{\Op()}{\Let{y}{\Op()}{f(x,y)}} \\
  &  \With x\,k.\,k\,\true ││ k\,\false \\
%≡\quad
%  &  ⟨\Let{\_}{\Log\,"working"}\Let{x}{\Op()}{\Let{y}{\Op()}{f(x,y)}} \\
%  &  / λx\,k.\,k\,\true ││ k\,\false ⟩\\
≡\quad
  &  ⟨\Log\,"working";\Let{x}{\S κ.\,\Ask(κ)\,()\,(λz.\,κ[z])}{\Let{y}{\Op()}{f(x,y)}} \\
  &  / λx\,k.\,k\,\true ││ k\,\false ⟩\\
→\quad
  &  ⟨\Log\,"working";\S κ.\,\Ask(κ)\,()\,(λz.\,κ[\Let{x}{z}{\Let{y}{\Op()}{f(x,y)}}]) \\
  &  / λx\,k.\,k\,\true ││ k\,\false ⟩\\
→^*\quad
  &  ⟨\Log\,"working";\S κ.\,\Ask(κ)\,()\,(λz.\,κ[\Let{y}{\Op()}{f(z,y)}]) \\
  &  / λx\,k.\,k\,\true ││ k\,\false ⟩\\
→^*\quad
  &  ⟨\Log\,"working";\A\,(
        ⟨\Let{y}{\Op()}{f(\true,y)} / h⟩
...
→^*\quad
  &  ⟨\Log\,"working";\A\,(
        ⟨⟨f(\true,\true)/h⟩ ││ ⟨f(\true, \false)/h⟩ / h⟩
     ││ ⟨⟨f(\false,\true)/h⟩ ││ ⟨f(\false, \false)/h⟩ / h⟩ \\
  &  / h⟩ \\
\end{align*}

%With $let.\S$ and $dL.\S$ we can derive the coarse-grained rewrites that started it all:
Finally, we can derive the coarse-grained rewrites that started it all:
...
Now we have to show that reduction doesn't change the result of a program.
...
\begin{lemma} Assume $e \Rrightarrow e'$. Then $e$ is a value iff $e'$ is a value.
\end{lemma}
...
We perhaps could have shown ${← · ↦} ⊆ {↦^= · ←^*}$ directly, without mentioning $\Rrightarrow$.
Our approach lets us reuse the already established substitution lemmas for $\Rrightarrow$.
...
This notion of contextual equivalence isn't the most common one.
We can deduce the usual notion using the argument of Biernacki et al.\  \cite{bisim}.
...
\end{theorem}
\begin{proof}
	By theorem \ref{ctxeqv1} it suffices to show that
...
The following $β$-equivalence shows that the structural substitution does the right thing:
\begin{center}
...
%since its equational theory (equivalence closure of reduction)
...
&→ ⟨\Let{x}{e}{\Let{y}{t}{\A\,t_r}}⟩ & let.let \\
&← ⟨\Let{x}{e}{\A\,⟨\Let{y}{t}{\A\,t_r}⟩}⟩ & \A.d \\
&≡ ⟨e │ x.\, ⟨t │ y.\,t_r⟩⟩
\end{align*}
...
&→ ⟨\S κ.\,\Let{k}{λx.\,κ⟦\Let{y}{x}{\A\,t_r}⟧}{e}⟩ & let.\S \\
&→ \Let{k}{λx.\,⟨\Let{y}{x}{\A\,t_r}⟩}{e} & d.\S \\
&→ \Let{k}{λy.\,⟨\A\,t_r⟩}{e} & let.v\\
&→ \Let{k}{λy.\,t_r}{e} & d.\S \\
&→ e\subst{k}{λy.\,t_r}
\end{align*}
...
&→ ⟨\Let{x}{e}{\Let{z}{t}{\A\,t_r}} / λy\,k.\,t_h⟩  \tagit{let.let} \\
...
%We also consider turning $\KwLet{}$s into delimiters unsatisfactory,
%as delimiters typically have a much higher runtime cost than $\KwLet{}$ bindings.
...
\end{equation*}
Expressible with $let.let$ and $let.id$: $\Let{x}{e}{E[x]} ←^* E[\Let{x}{e}{x}] → E[e]$.
...
The equality for handling shouldn't be surprising (operation with no payload):
%Handling of algebraic operations can be characterized by equalities \cite{handlers}, viz.
$$\Handle \Op(\overline{v_i}) \With H = t\overline{\subst{k_i}{λx.\,\Handle v_i\;x \With H}}$$
...
$$\Op v ≡ \shiftz\,k.\,h\;v\;k \quad\quad \Handle e \With x\,k.\,t ≡ d\Let{h}{λx\,k.\,t}{⟨e⟩}$$
or above:
$$\Op v ≡ \Let{h'}{h}{\shiftz\,k.\,h'\;v\;k} \quad\quad \Handle e \With x\,k.\,t ≡ ⟨d\Let{h}{λx\,k.\,t}{e}⟩$$
...
The problem with dynamic parameters $p$ and their binders $d\Let{p}{e}{t}$ is
the same as with conventional syntax for operations and handlers:
...
many early attempts for the $λ$-calculus were flawed.
...
Formally, the $let.\S$ reduction is allowed only for impure lets (without the assertion)
and $E$ contexts are composed of impure lets.
We could add reductions such as $κ⟦\Letp{x}{e}{t}⟧ → \Letp{x}{e}{κ⟦t⟧}$.
...
and a map from labels to the ``dynamically bound'' values.
...
preserve typability when translating (figure \ref{shorthands})
from conventional type-and-effect systems for algebraic effects.

\section{Tail aborts}
In general, delimited control operators search for the delimiter using a loop,
but we know than an $\abort$ in the tail position of a delimiter
will pop exactly one stack frame.
This is a low-level optimization that can't be expressed in our calculus.
Perhaps this instance of local control has something to do with join points.
