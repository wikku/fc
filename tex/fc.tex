...
The conventional evaluation step rule is \cite{leijen}:
...
$\shiftz$. The conventional rule for $\shiftz$ and its delimiter \textsf{reset} (written $⟨·⟩$) is \cite{subtyping}:
...
\textit{pure} ($E$) and \textit{delimited} ($D$) evaluation contexts.\footnote{
	An evaluation context is a particular syntactic manifestation of the broader concept of a continuation.
	%To wit, we could mean continuations informally, or represented as functions in CPS denotational semantics, or with a particular optimized representation in an abstract machine...
}
