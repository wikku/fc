...
What follows is a formal definition of our calculus, its reduction rules, and their example applications (Chapter \ref{Syntax}).
We then prove that the reductions are correct in that they preserve operational semantics (Chapter \ref{Adequacy}).
Chapter \ref{Confluence} is about confluence, the freedom to apply reductions in any way we want.
We are confident in our results thanks to machine-verified proofs, described in Chapter \ref{Abella}.
This work builds on many others, which we elaborate upon in Chapter \ref{Related}.
Finally, we discuss possible extensions and modifications in Chapter \ref{Future}.
...
\textit{pure} ($E$) and \textit{delimited} ($D$) evaluation contexts.\footnote{
	We use evaluation contexts as syntactic representations of the broader concept of continuations.
}
...
The latter is defined as follows on the two expression formers that use $κ$:
\begin{enumerate}
	\item
...
Because $\A$ removes any surrounding context $E$, it will naturally most often be seen in a tail position.
In the tail position of a delimiter, its function is removing only that delimiter.
We will see this pattern arise naturally in the course of reduction.
The fourth shorthand lets us omit data in delimiters ($╱v$) when irrelevant.
...
(another instance of the pattern mentioned previously).
...
We therefore substitute that delimiter for $κ$. This makes the bound variable unused,
therefore $\S$ can be replaced with $\A$ for concision.
...
We will call this meaning preservation property \textit{adequacy} \cite{crary2009}.
...
more specifically zero or more evaluation steps followed by
...
letting us induct on size in the proof of the diamond property.
This is no longer true if we add data to delimiters, therefore some other solution is needed
if going for a proof through the diamond property.
...
but the natural way to check if something matches that pattern is to (superparallel) reduce it first
...
Abella follows the tradition of Church's Simple Theory of Types \cite{Church}
...
This embodies Perlis's epigram ``There is no such thing as a free variable'' \cite{Perlis}:
...
Indeed, it's not available in Abella's \textit{specification logic} $λ$-Prolog \cite{Nadathur}.
...
Our equational theory is broadly speaking no less powerful, except we eschew $η$ rules since
they sometimes change operational behavior.
Naturally, equations are undirected and it isn't always obvious in which direction we should rewrite
to expose the most optimizations.
...
as ``rebindable top-levels'' $\hat{\mathsf{tp}}$.
Their key addition to the $λμ$-calculus is the peculiar reduction that appears to peek under a binder
(\foreign{$μ \hat{\mathsf{tp}}.\,⟦\hat{\mathsf{tp}}⟧v ↦ v$}).
They consider only operational rules, rather than observational equivalences or optimizations.
In this approach, the encodings of operators such as $\shiftz$ end up rather complex.

In another paper from the same year \cite{compositional},
instead of using Parigot's original syntax they use de Groote's more
...
to be a command.
They show that after this relaxation, the concept of $\mathsf{tp}$ variables is unnecessary.
In intermediate steps, they rely upon $\tilde{μ}$, an operator ``opposite'' to $μ$.
It is used for building \textit{co-terms}, which can be plugged just like continuation variables,
in effect making delimiters with return clauses.
It's treated as syntax sugar, since it can be expressed using the other language constructs similarly
to our encoding of return clauses (Figure \ref{shorthands}),
namely with a $μ$-abstraction that ignores its argument in the tail position of a plug.
In the end, they show equational correspondence of their calculus $Λμ_v$ with $λ_\$$.
In their encoding, reset $⟨e⟩$ is essentially expressed as $κ⟦\Let{x}{e}{\A\,x}⟧$, where
$κ$ is a fresh variable.
While technically correct, it's perhaps not the most natural approach and it's not as easily
extended with data in delimiters.
...
\section{Tail reductions and letcc} \label{tailred}
...
Type-and-effect systems for $\shiftz$ are highly complex \cite{subtyping, fscd19},
and would be even more so with our generalization to $\S$ and the addition of data to delimiters.
Any future work in this direction ideally should
preserve typability when translating (as in Figure \ref{shorthands})
from some existing type system for algebraic effects \cite{leijen, hwc}.
